...
public:
...

	{
		...
		// Insert K numbers from start in maxheap
		...
		// For the remaining number push them in the maxheap if number < heap.top() and simultaneously pop

		for (int j=k;j<nums.size();j++)
		{
			if(nums[j]<maxHeap.top())
			{
				maxHeap.pop();
				maxHeap.push(nums[j]);
